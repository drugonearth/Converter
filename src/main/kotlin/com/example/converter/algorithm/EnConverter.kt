...
class EnConverter : Converter(){
    ...
    override fun stringToNum(input: String): String {
        ...
        if(arr.isNotEmpty()) throw ConverterInputException()
    ...
    private fun numSubstringBuilder(steps: Int, arr: ArrayList<String>, output: StringBuilder) //steps максимальная степень 1000
    {
        ...
        for(i in 1..steps)
        {
            var searched = searchWords(arr, output, i)

            if(searched == 2 && arr.isNotEmpty() && i!=steps)
                output.insert(0,"000")
            else {
                search1for1000(arr, output, searched)
            }

            if(searched == 0 && arr.isNotEmpty() && i!=steps)
                output.insert(0,"00")
        }
    }
        ...
        for(i in 0..1)
        {
            if(temp==digitMap[nulls]?.get(i))
            {
                ...
                if(arr.isEmpty())
                {
                    ...
                    else throw ConverterInputException()
                }
                return i
            }
        }
    ...
    private fun search1for1000(arr: ArrayList<String>, sb:StringBuilder, pos: Int) //1-19 для тысяч, миллионов, миллиардов
    {
        if(arr.isEmpty()) return
